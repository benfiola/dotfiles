# coding=utf-8
# from: https://raw.githubusercontent.com/octplane/ansible_stdout_compact_logger/8412eb40bc96773e80b8e43c1019d182785fc74c/callbacks/anstomlog.py
...
def deep_serialize(data, indent=0):
    ...
    if isinstance(data, list):
        ...
        if len(data) == 1:
            ...
            for item in data:
                output = (
                    output + "\n" + list_padding + "- " + deep_serialize(item, indent)
                )
            output = output + "\n" + padding + " ]"
            ...
            if key in data.keys():
                ...
                output = (
                    ...
                    % "\n".join(
                        [
                            " " * len(prefix) + line
                            for line in deep_serialize(value, indent).splitlines()
                        ]
                    ).strip()
                )
...
class TestStringMethods(unittest.TestCase):
    ...
    test_structure = {
        "cmd": ["false"],
        "end": "2016-12-29 16:46:04.151591",
        "_ansible_no_log": False,
        "stdout": "",
        "changed": True,
        "failed": True,
        "delta": "0:00:00.005046",
        "stderr": "",
        "rc": 1,
        "invocation": {
            "module_name": "command",
        ...
        "stdout_lines": [],
        "start": "2016-12-29 16:46:04.146545",
        "warnings": [],
    }

    def test_single_item_array(self):
        self.assertEqual(deep_serialize(self.test_structure["cmd"]), "[ false ]")

    def test_single_empty_item_array(self):
        self.assertEqual(deep_serialize([""]), '[ "" ]')

    def test_issue_4(self):
        self.assertEqual(deep_serialize(["ÉLÉGANT"]), "[ ÉLÉGANT ]")

    def test_empty_array(self):
        self.assertEqual(deep_serialize(self.test_structure["stdout_lines"]), "[]")
...
class CallbackModule(CallbackBase):
    ...
    def v2_runner_on_failed(self, result, ignore_errors=False):
        ...
        self._emit_line(
            "%s | FAILED | %s" % (host_string, duration), color=C.COLOR_ERROR
        )
    ...
    def v2_runner_on_ok(self, result):
        ...
        if (
            ...
            for item in result._result["results"]:
                ...
                self._emit_line("%s | %s" % (item_msg, duration), color=color)
        else:
    ...
    def v2_runner_on_skipped(self, result):
        ...
        self._emit_line(
            "%s | SKIPPED | %s" % (self._host_string(result), duration),
            color=C.COLOR_SKIP,
        )
