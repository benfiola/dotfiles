  ...
  nixos =
    ...
    lib.mkIf config.padctl.enable {
      ...
      services.udev.extraRules = ''
        ...
        # padctl claims the vendor-class interfaces via libusb, which needs
        # write access to the raw USB device node too, not just hidraw.
        ...
        # xpad binds the pad's XInput interface before padctl can claim it.
        # Unbind only while a padctl daemon is actually running (its control
        # socket exists), so disabling padctl restores plain kernel gamepad
        # support instead of leaving the interface driverless.
